         ...
         if (env.ARTIFACTORY == "ECR") {
             ...
             for (int i = 0; i < list.size(); i++) {
               ...
               if ("${list[i]}" == "'UnitTests'" && env.ACTION == 'DEPLOY') {
                 stage('Unit Tests') {
                    sh """
                    docker run --rm --user root -v "$WORKSPACE":/opt/repo -w /opt/repo $NODE_IMAGE /bin/bash -c "cd /opt/repo && yarn install && yarn test -- --coverage --watchAll=false && yarn run test:coverage"
